...
class StartCommand():
    ...
    async def sign_up_refferal(
        ...
        await message.edit_text(
            f"{message.text}\n\n>>> 🤵‍♂️Вы были приглашены пользователем <a h"
            f"ref='{await get_link(refferal_id)}'>{refferal_mask}"
            f"{refferal_nick}</a>",
        )
    ...
    async def _register_refferal(self, message: Message, ref_id: int | str):
        ...
        await message.answer(
            (
                f"🦎 Привет, {message.from_user.full_name}! Добро пожаловать в "
                "Живополис, лучший игровой бот во всея телеграмме! \n\n⚙️ Наж"
                "мите на кнопку ниже для того, чтобы создать аккаунт и начать "
                "играть…"
            ),
    ...
    async def _user_register_message(
        ...
        if not refferal_link:
            await bot.send_message(
                user_id,
                text=(
                    f"🦎 Привет, {message.from_user.full_name}! Добро п"
                    "ожаловать в Живополис, лучший игровой бот во всея "
                    "телеграмме! \n\n⚙️ Нажмите на кнопку ниже для того,"
                    " чтобы создать аккаунт и начать играть…"
                ),
                reply_markup=markup
            )
            return
        ...
        if inviter < 1:
            await bot.send_message(
                user_id,
                text=(
                    f"🦎 Привет, {message.from_user.full_name}! Добро по"
                    "жаловать в Живополис, лучший игровой бот во всея те"
                    "леграмме! \n\n⚙️ Нажмите на кнопку ниже для того, что"
                    "бы создать аккаунт и начать играть…"
                ),
                reply_markup=markup
            )
        elif inviter > 1:
    ...
    async def _casino_start(self, message: Message):
        ...
        await message.answer(
            (
                "🎮 Добро пожаловать в игровой клуб! Здесь вы можете повесе"
