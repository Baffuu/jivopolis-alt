...
try:
    ...
    async def on_startup(_):
        try:
            await main.send_message(fid, '✅ <i>Бот перезагружен\n#bot_reload</i>', parse_mode='html')
        except:
            pass
    async def db_table_val(user_id, user_name, user_surname, username, balance, cansteal, nick, bio, rasa, temp, morj, mask, ready, battles, chn):
        cursor.execute('INSERT INTO userdata (user_id, user_name, user_surname, username, balance, cansteal, nick, desc, rasa, temp, morj, mask, ready, battles, clanname) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', (user_id, user_name, user_surname, username, balance, cansteal, nick, bio, rasa, temp, morj, mask, ready, battles, chn,))
        conn.commit()
    def current_time():
    ...
    def remaining(type):
        ...
        min = seconds//60
        sec = seconds%60
        return '{0}{1}'.format('{0} минут '.format(min) if min!=0 else '', '{0} секунд'.format(sec) if sec!=0 else '')
    async def check(user, chat):
        try:
            ...
            if diff>=86400:
                for lim in limiteds:
                    cursor.execute("UPDATE globaldata SET {0}=?".format(lim), (random.randint(5,15),))
                    conn.commit()
                cursor.execute("UPDATE globaldata SET lastfill=?", (current_time(),))
            a = user
            cursor.execute('UPDATE userdata SET lastseen=? WHERE user_id=?', (current_time(),a,))
            conn.commit()
            cursor.execute('SELECT points FROM userdata WHERE user_id=?', (a,))
            points = cursor.fetchone()[0]
            if user == CREATOR:
                cursor.execute('UPDATE userdata SET rang=3 WHERE user_id=?', (a,))
                conn.commit()
            cursor.execute('SELECT lastelec FROM userdata WHERE user_id=?', (a,))
            lastelec = cursor.fetchone()[0]
            if current_time() - lastelec > 24*3600:
                cursor.execute('UPDATE userdata set electimes=0 WHERE user_id=?', (a,))
                conn.commit()
                cursor.execute('UPDATE userdata set lastelec=? WHERE user_id=?', (current_time(), a,))
                conn.commit()
            cursor.execute('SELECT lvl FROM userdata WHERE user_id=?', (a,))
            lvl = cursor.fetchone()[0]
            if lvl>=len(levelrange)-1:
                return
            if points>=levelrange[lvl] and points<levelrange[lvl+1]:
                return
            for i in levelrange:
                if points>=i and levelrange.index(i)>=len(levelrange)-1 and lvl!=levelrange.index(i):
                    cursor.execute('UPDATE userdata SET lvl=? WHERE user_id=?', (levelrange.index(i), a,))
                    conn.commit()
                    try:
                        await main.send_message(user, '<i>&#128305; Теперь ваш уровень в Живополисе: <b>{0}</b>\nПоздравляем!\n{1}</i>'.format(levelrange.index(i), leveldesc[levelrange.index(i)]), parse_mode='html')
                        return
                    except Exception as e:
                        await main.send_message(chat, '<i>&#128305; Теперь ваш уровень в Живополисе: <b>{0}</b>\nПоздравляем!\n{1}</i>'.format(levelrange.index(i), leveldesc[levelrange.index(i)]), parse_mode='html')
                        return
            ...
            if 'NoneType' in str(e):
                ...
                print(e)
    async def achieve(user, chat, achi):
        try:
            a = user
            ...
            money = ach[3][ind]
            points = ach[4][ind]
            cursor.execute('UPDATE userdata SET {0} = 1 WHERE user_id = ?'.format(achi), (a,))
            conn.commit()
            ...
            conn.commit()
            ch = await main.get_chat(chat)
            if ch.type=='private':
                await main.send_message(chat, '<i>У вас новое достижение: <b>{0}</b>\n{1}. \nВаша награда: <b>${2}</b> и &#128161; <b>{3}</b> очков</i>'.format(name, desc, money, points), parse_mode = 'html')
            else:
                await main.send_message(chat, '<i><b><a href="tg://user?id={0}">{1}{2}</a></b>, у вас новое достижение: <b>{3}</b>\n{4}. \nВаша награда: <b>${5}</b> и &#128161; <b>{6}</b> очков</i>'.format(a, rasa, nick, name, desc, money, points), parse_mode = 'html')
        except Exception as e:
            await main.send_message(chat, '&#10060; <i>При выполнении команды произошла ошибка. Проверьте, есть ли у вас аккаунт в Живополисе. Если вы выполняли действие над другим пользователем, проверьте, есть ли у этого пользователя аккаунт в Живополисе. Помните, что выполнение действий над ботом Живополиса невозможно.\nЕсли ошибка появляется даже когда у вас есть аккаунт, возможно, проблема в коде Живополиса. Сообщите о ней в Приёмную (t.me/zhivolab), и мы постараемся исправить проблему.\nИзвините за предоставленные неудобства</i>', parse_mode='html')
            await main.send_message(chat, '<i><b>Текст ошибки: </b>{0}</i>'.format(e), parse_mode = 'html')
    async def eat(call, foodname, healthid):
        a = call.from_user.id
        chat = call.message.chat.id
        try:
            cursor.execute('SELECT health FROM userdata WHERE user_id=?', (a,))
            health = cursor.fetchone()[0]
            cursor.execute('SELECT {0} FROM userdata WHERE user_id=?'.format(foodname), (a,))
            foodib = cursor.fetchone()[0]
            if foodib <1:
                await call.answer(text='❌ У вас нет такой еды', show_alert = True)
                return
            cursor.execute('UPDATE userdata SET {0}={0}-1 WHERE user_id=?'.format(foodname), (a,))
            conn.commit()
            cursor.execute('UPDATE userdata SET health=health+? WHERE user_id=?', (healthid,a,))
            conn.commit()
            if healthid>0:
                await call.answer(text='❤ +{0} HP на здоровье!'.format(healthid), show_alert = True)
            else:
                await call.answer(text='🤢 Зачем я это съел? Теперь мне нехорошо', show_alert = True)
                cursor.execute('SELECT health FROM userdata WHERE user_id=?', (a,))
                health = cursor.fetchone()[0]
                if health<=0:
                    await main.send_message(chat, '<i>&#9760; Вы умерли</i>', parse_mode='html')
        except Exception as e:
            await main.send_message(chat, '&#10060; <i>При выполнении команды произошла ошибка. Проверьте, есть ли у вас аккаунт в Живополисе. Если вы выполняли действие над другим пользователем, проверьте, есть ли у этого пользователя аккаунт в Живополисе. Помните, что выполнение действий над ботом Живополиса невозможно.\nЕсли ошибка появляется даже когда у вас есть аккаунт, возможно, проблема в коде Живополиса. Сообщите о ней в Приёмную (t.me/zhivolab), и мы постараемся исправить проблему.\nИзвините за предоставленные неудобства</i>', parse_mode='html')
            await main.send_message(chat, '<i><b>Текст ошибки: </b>{0}</i>'.format(e), parse_mode = 'html')
    async def cure(a, oth, chat):
        try:
            nerr = 0
            cursor.execute('SELECT medicine FROM userdata WHERE user_id=?', (a,))
            medicine = cursor.fetchone()[0]
            if medicine<=0:
                await main.send_message(chat, '<i>&#10060; У вас нет таблеток(</i>', parse_mode='html')
                return
            cursor.execute('SELECT health FROM userdata WHERE user_id=?', (oth,))
            health = cursor.fetchone()[0]
            cursor.execute('SELECT nick FROM userdata WHERE user_id=?', (a,))
            nick = cursor.fetchone()[0]
            cursor.execute('SELECT rasa FROM userdata WHERE user_id=?', (a,))
            ...
            if health > 0 and health < 100:
                cursor.execute('UPDATE userdata SET medicine=medicine-1 WHERE user_id=?', (a,))
                conn.commit()
                rand = random.randint(1,100-health)
                cursor.execute('UPDATE userdata SET health=health+? WHERE user_id=?', (rand, oth,))
                conn.commit()
                cursor.execute('SELECT nick FROM userdata WHERE user_id=?', (a,))
                nick = cursor.fetchone()[0]
                cursor.execute('SELECT rasa FROM userdata WHERE user_id=?', (a,))
                ...
                orasa = cursor.fetchone()[0]
                if oth==a:
                    await main.send_message(chat, '<i>&#128138; Успех! Вы вылечили себя</i>', parse_mode='html')
                    return
                else:
                    await main.send_message(chat, '<i>&#128138; Успех! Вы вылечили <b><a href="tg://user?id={0}">{1}{2}</a></b></i>'.format(oth, orasa, onick), parse_mode='html')
                    await main.send_message(oth, '<i>&#128138; Вас вылечил <b><a href="tg://user?id={0}">{1}{2}</a></b></i>'.format(a, rasa, nick), parse_mode='html')
                    nerr = 1
            elif health>=100:
                if oth!=a:
                    await main.send_message(chat, '<i>&#128138; <b><a href="tg://user?id={0}">{1}{2}</a></b> полностью здоров, зачем вам тратить лекарства впустую?\nЛекарства <b>не потрачены</b></i>'.format(oth, orasa, onick), parse_mode='html')
                    return
                else:
                    await main.send_message(chat, '<i>&#128138; Вы полностью здоровы, зачем вам тратить лекарства впустую?\nЛекарства <b>не потрачены</b></i>', parse_mode='html')
                    return
            else:
                if oth==a:
                    await main.send_message(chat, '<i>&#10060; Вы не можете воскресить самого себя</i>', parse_mode='html')
                    return
                cursor.execute('UPDATE userdata SET medicine=medicine-1 WHERE user_id=?', (a,))
                conn.commit()
                rand = random.randint(50,100)
                cursor.execute('UPDATE userdata SET health=? WHERE user_id=?', (rand, oth,))
                conn.commit()
                cursor.execute('SELECT nick FROM userdata WHERE user_id=?', (a,))
                nick = cursor.fetchone()[0]
                cursor.execute('SELECT rasa FROM userdata WHERE user_id=?', (a,))
                ...
                cursor.execute('SELECT rasa FROM userdata WHERE user_id=?', (oth,))
                orasa = cursor.fetchone()[0]
                await main.send_message(chat, '<i>&#128138; Успех! Вы воскресили <b><a href="tg://user?id={0}">{1}{2}</a></b></i>'.format(oth, orasa, onick), parse_mode='html')
                nerr = 1
                await main.send_message(oth, '<i>&#128138; Вас воскресил <b><a href="tg://user?id={0}">{1}{2}</a></b></i>'.format(a, rasa, nick), parse_mode='html')
                await achieve(a, chat, 'helper')
            if nerr==1:
                cursor.execute("UPDATE userdata SET cured=cured+1 WHERE user_id=?", (a,))
                conn.commit()
                cursor.execute("SELECT cured FROM userdata WHERE user_id=?", (a,))
                cured = cursor.fetchone()[0]
                if cured>=20:
                    await achieve(a, chat, 'medquest')
                    cursor.execute("UPDATE userdata SET medic=medic+1 WHERE user_id=?", (a,))
                    await main.send_message(chat, '<i>Вы получаете <b>🩺 Стетоскоп</b>. Эта маска будет показывать всем, что вы профессиональный врач, и вам можно доверять</i>', parse_mode = 'html')
                    conn.commit()
        except Exception as e:
            await main.send_message(chat, '&#10060; <i>При выполнении команды произошла ошибка. Проверьте, есть ли у вас аккаунт в Живополисе. Если вы выполняли действие над другим пользователем, проверьте, есть ли у этого пользователя аккаунт в Живополисе. Помните, что выполнение действий над ботом Живополиса невозможно.\nЕсли ошибка появляется даже когда у вас есть аккаунт, возможно, проблема в коде Живополиса. Сообщите о ней в Приёмную (t.me/zhivolab), и мы постараемся исправить проблему.\nИзвините за предоставленные неудобства</i>', parse_mode='html')
            await main.send_message(chat, '<i><b>Текст ошибки: </b>{0}</i>'.format(e), parse_mode = 'html')
    async def poison(a, oth, chat):
        try:
            cursor.execute('SELECT health FROM userdata WHERE user_id=?', (a,))
            myhealth = cursor.fetchone()[0]
            if myhealth <=0:
                await main.send_message(chat, '<i>&#9760; Вы умерли. Попросите кого-нибудь вас воскресить</i>', parse_mode = 'html')
                return
            cursor.execute('SELECT poison FROM userdata WHERE user_id=?', (a,))
            poison = cursor.fetchone()[0]
            if poison<=0:
                await main.send_message(chat, '<i>&#10060; У вас нет яда. Возможно, это к лучшему</i>', parse_mode='html')
                return
            cursor.execute('SELECT health FROM userdata WHERE user_id=?', (oth,))
            health = cursor.fetchone()[0]
            cursor.execute('UPDATE userdata SET poison=poison-1 WHERE user_id=?', (a,))
            conn.commit()
            cursor.execute('SELECT nick FROM userdata WHERE user_id=?', (a,))
            nick = cursor.fetchone()[0]
            cursor.execute('SELECT rasa FROM userdata WHERE user_id=?', (a,))
            ...
            rt = random.choice([0, 1])
            if rt==0:
                cursor.execute('UPDATE userdata SET health=health-? WHERE user_id=?', (rand, oth,))
                conn.commit()
                await main.send_message(fid, '<i><b><a href="tg://user?id={0}">{1}{2}</a></b> отравил <b><a href="tg://user?id={3}">{4}{5}</a></b>\n#user_poison</i>'.format(a, rasa, nick, oth, orasa, onick), parse_mode='html')
                await main.send_message(chat, '<i>&#129514; Вы отравили <b><a href="tg://user?id={0}">{1}{2}</a></b></i>'.format(oth, orasa, onick), parse_mode='html')
                await main.send_message(oth, '<i>&#129514; Вас отравил <b><a href="tg://user?id={0}">{1}{2}</a></b></i>'.format(a, rasa, nick), parse_mode='html')
            else:
                await main.send_message(chat, '<i>&#10060; Неудача. Возможно, это к лучшему.\nЯд потрачен зря</i>', parse_mode='html')
        except Exception as e:
            await main.send_message(chat, '&#10060; <i>При выполнении команды произошла ошибка. Проверьте, есть ли у вас аккаунт в Живополисе. Если вы выполняли действие над другим пользователем, проверьте, есть ли у этого пользователя аккаунт в Живополисе. Помните, что выполнение действий над ботом Живополиса невозможно.\nЕсли ошибка появляется даже когда у вас есть аккаунт, возможно, проблема в коде Живополиса. Сообщите о ней в Приёмную (t.me/zhivolab), и мы постараемся исправить проблему.\nИзвините за предоставленные неудобства</i>', parse_mode='html')
            await main.send_message(chat, '<i><b>Текст ошибки: </b>{0}</i>'.format(e), parse_mode = 'html')
    async def shoot(a, oth, chat):
        try:
            cursor.execute('SELECT health FROM userdata WHERE user_id=?', (a,))
            myhealth = cursor.fetchone()[0]
            if myhealth <=0:
            ...
            gun = cursor.fetchone()[0]
            if gun<=0:
                await main.send_message(chat, '<i>&#10060; У вас нет пистолета. Возможно, это к лучшему</i>', parse_mode='html')
                return
            cursor.execute('SELECT health FROM userdata WHERE user_id=?', (oth,))
            health = cursor.fetchone()[0]
            cursor.execute('UPDATE userdata SET pistol=pistol-1 WHERE user_id=?', (a,))
            conn.commit()
            cursor.execute('SELECT nick FROM userdata WHERE user_id=?', (a,))
            nick = cursor.fetchone()[0]
            cursor.execute('SELECT rasa FROM userdata WHERE user_id=?', (a,))
            ...
            if rt==0:
                cursor.execute('UPDATE userdata SET health=health-? WHERE user_id=?', (rand, oth,))
                conn.commit()
                await main.send_message(fid, '<i><b><a href="tg://user?id={0}">{1}{2}</a></b> застрелил <b><a href="tg://user?id={3}">{4}{5}</a></b>\n#user_gunshoot</i>'.format(a, rasa, nick, oth, orasa, onick), parse_mode='html')
                await main.send_message(chat, '<i>&#128299; Вы застрелили <b><a href="tg://user?id={0}">{1}{2}</a></b></i>'.format(oth, orasa, onick), parse_mode='html')
                await main.send_message(oth, '<i>&#128299; Вас застрелил <b><a href="tg://user?id={0}">{1}{2}</a></b></i>'.format(a, rasa, nick), parse_mode='html')
                rt = random.choice([0, 1])
                if rt==1:
                    cursor.execute('UPDATE userdata SET prison=? WHERE user_id=?', (current_time()+1200, a,))
                    conn.commit()
                    await main.send_message(chat, '<i>&#128110; Господин <b><a href="tg://user?id={0}">{1}{2}</a></b>, вы задержаны за убийство огнестрельным оружием. Пройдёмте в отделение.\n\nВы были арестованы на <b>20 минут</b></i>'.format(a, rasa, nick), parse_mode='html')
            else:
                await main.send_message(chat, '<i>&#10060; Вы выстрелили мимо. Возможно, это к лучшему.\nПистолет потрачен зря</i>', parse_mode='html')
        except Exception as e:
            await main.send_message(chat, '&#10060; <i>При выполнении команды произошла ошибка. Проверьте, есть ли у вас аккаунт в Живополисе. Если вы выполняли действие над другим пользователем, проверьте, есть ли у этого пользователя аккаунт в Живополисе. Помните, что выполнение действий над ботом Живополиса невозможно.\nЕсли ошибка появляется даже когда у вас есть аккаунт, возможно, проблема в коде Живополиса. Сообщите о ней в Приёмную (t.me/zhivolab), и мы постараемся исправить проблему.\nИзвините за предоставленные неудобства</i>', parse_mode='html')
            await main.send_message(chat, '<i><b>Текст ошибки: </b>{0}</i>'.format(e), parse_mode = 'html')
    async def create_acc(user, chat):
        try:
            cursor.execute('SELECT COUNT(*) FROM userdata WHERE user_id=?', (user.id,))
            count = cursor.fetchone()[0]
            if count>0:
                await main.send_message(chat, '<i>Вы уже создавали аккаунт</i>', parse_mode = 'html', reply_markup = types.ReplyKeyboardRemove())
                return
            await db_table_val(user_id=user.id, user_name=user.first_name, user_surname=user.last_name, username=user.username, balance=0, cansteal=1, nick = user.first_name, bio = 'пусто', rasa = '&#128529;', temp = '&#128529;', morj = 0, mask = '', ready = 0, battles = 0, chn = '')
            conn.commit()
            await main.send_message(fid, '<i><b><a href="tg://user?id={0}">{1}</a></b> присоединился(-ась) к Живополису\n#user_signup</i>'.format(user.id, user.first_name), parse_mode='html')
            cursor.execute('UPDATE userdata SET frstseen = ? WHERE user_id=?', (current_time(), user.id,))
            conn.commit()
        except Exception as e:
            hideBoard = types.ReplyKeyboardRemove()
            if str(e).startswith('UNIQUE constraint failed: '):
                await main.send_message(chat, '<i>Вы уже создавали аккаунт</i>', parse_mode = 'html', reply_markup = hideBoard)
            elif str(e) == 'database is locked':
                await main.send_message(chat, '<i><b>&#10060; Ошибка: </b>база данных заблокирована</i>', parse_mode = 'html', reply_markup = hideBoard)
            else:
                await main.send_message(chat, f'<i><b>&#10060; Ошибка: </b>{e}</i>', parse_mode = 'html', reply_markup = hideBoard);
        else:
            hideBoard = types.ReplyKeyboardRemove()
            await main.send_message(chat, '<i>&#9989; Вы были добавлены в базу данных</i>', parse_mode = 'html', reply_markup = hideBoard);
    ...
    async def chats(user, message):
        try:
            cursor.execute('SELECT temp FROM userdata WHERE user_id = ?', (user,))
            temp = cursor.fetchone()[0]
            if temp == '&#128049':
            ...
            markup = types.InlineKeyboardMarkup()
            if chat != '':
                markup.add(types.InlineKeyboardButton(text=chat, url=url))
            else:
                markup.add(types.InlineKeyboardButton(text='Выбрать расу', callback_data='change_rasa'))
            markup.add(types.InlineKeyboardButton(text='🎮 Игровой клуб', url='https://t.me/+2UuPwVyac6lkYjRi'))
            ...
            await message.answer('&#10060; <i>При выполнении команды произошла ошибка. Проверьте, есть ли у вас аккаунт в Живополисе. Если вы выполняли действие над другим пользователем, проверьте, есть ли у этого пользователя аккаунт в Живополисе. Помните, что выполнение действий над ботом Живополиса невозможно.\nЕсли ошибка появляется даже когда у вас есть аккаунт, возможно, проблема в коде Живополиса. Сообщите о ней в Приёмную (t.me/zhivolab), и мы постараемся исправить проблему.\nИзвините за предоставленные неудобства</i>', parse_mode='html')
            await message.answer('<i><b>Текст ошибки: </b>{0}</i>'.format(e), parse_mode = 'html')
    async def profile(user, message, called=False):
        try:
            a = user
            cursor.execute('SELECT nick FROM userdata WHERE user_id = ?', (a,))
            nick = cursor.fetchone()[0]
            ...
            ready = cursor.fetchone()[0]
            if ready == 'private' and a!=message.from_user.id and not called:
                await message.answer('&#128683; <i><b><a href="tg://user?id={2}">{0}{1}</a></b> скрыл свой профиль</i>'.format(rasa, nick, a), parse_mode = 'html')
                return
            cursor.execute('SELECT balance FROM userdata WHERE user_id=?', (a,))
            money = cursor.fetchone()[0]
            cursor.execute('SELECT refid FROM userdata WHERE user_id=?', (a,))
            refid = cursor.fetchone()[0]
            refca = ''
            if refid!=0:
                cursor.execute('SELECT nick FROM userdata WHERE user_id = ?', (refid,))
                rnick = cursor.fetchone()[0]
                cursor.execute('SELECT rasa FROM userdata WHERE user_id = ?', (refid,))
                rrasa = cursor.fetchone()[0]
                refca = '\n📎 Пригласивший пользователь: <b><a href="tg://user?id={0}">{1}{2}</a></b>'.format(refid,rrasa,rnick)
            cursor.execute('SELECT nick FROM userdata WHERE user_id=?', (a,))
            nick = cursor.fetchone()[0]
            cursor.execute('SELECT rasa FROM userdata WHERE user_id=?', (a,))
            rasa = cursor.fetchone()[0]
            ...
            cursor.execute('SELECT lvl FROM userdata WHERE user_id=?', (a,))
            lvl = cursor.fetchone()[0]
            cursor.execute('SELECT lastseen FROM userdata WHERE user_id=?', (a,))
            lastseen = cursor.fetchone()[0]
            cursor.execute('SELECT frstseen FROM userdata WHERE user_id=?', (a,))
            ...
            if rang == 0:
                ...
                rangs = '&#129332; Создатель'
            if clan!=0:
                cursor.execute('SELECT username FROM clandata WHERE group_id=?', (clan,))
                usn = cursor.fetchone()[0]
                cursor.execute('SELECT name FROM clandata WHERE group_id=?', (clan,))
                chn = cursor.fetchone()[0]
                cursor.execute('SELECT type FROM clandata WHERE group_id=?', (clan,))
                typ = cursor.fetchone()[0]
            seconds = current_time()-lastseen
            y = floor(seconds / 31536000);
            mo = floor((seconds % 31536000) / 2628000);
            mos = str(mo)
            mos = mos[len(mos)-1]
            d = floor(((seconds % 31536000) % 2628000) / 86400);
            ds = str(d)
            ds = ds[len(ds)-1]
            ...
            if hs =='1':
                hs = 'час'
            elif hs== '2' or hs == '3' or hs == '4':
                hs = 'часа'
            else:
                hs = 'часов'
            if ms=='1':
                ms = 'минута'
            elif ms== '2' or ms == '3' or ms == '4':
                ms = 'минуты'
            else:
                ms = 'минут'
            if mo!=0 and y==0:
                last+='{0} {1} '.format(mo, mos)
            if d!=0 and mo==0 and y==0:
                last+='{0} {1} '.format(d, ds)
            if h!=0 and y==0 and d==0 and mo==0 and y==0:
                last+='{0} {1} '.format(h, hs)
            if m!=0 and h==0 and y==0 and mo==0 and y==0:
                last+='{0} {1} '.format(m, ms)
            last+='назад'
            if last=='назад':
                last='только что'
            if y>0:
            ...
            first = '{0} {1} {2}'.format(day, month, year)
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton(text='💡 Достижения', callback_data='achievements'))
            markup.add(types.InlineKeyboardButton(text='⚙ Настройки', callback_data='user_settings'))
            markup.add(types.InlineKeyboardButton(text='🖇 Реферальная ссылка', callback_data='reflink'))
            markup.add(types.InlineKeyboardButton(text='👥 Привлечённые пользователи', callback_data='refusers'))
            prof = '<i><b><a href="tg://user?id={2}">{0}{1}</a></b>\n<b>{8}</b>\n<b>Был(-а)</b> {10}\n<b>Аккаунт создан:</b> {11}{12}\n&#128176; Баланс: <b>${3}</b>\n&#128221; Описание: \n<b>{4}</b>\n&#9876; Режим готовности: <b>{5}</b>\n&#128305; Уровень: {6}\n&#128737; Клан: <b>{7}</b>\n&#128138; Здоровье: {9}</i>'.format(rasa, nick, a, money, bio, 'не готов' if ready == 0 else 'готов', '<b>{0}</b> ({1} {2})'.format(lvl, points, rem), ('<a href="{1}">{0}</a>'.format(chn, usn) if typ == 'public' else chn) if clan != 0 else 'отсутствует', rangs, health, last, first, refca)
            if photo == "":
                await message.answer(prof, parse_mode = 'html', reply_markup = markup)
            else:
                try:
                    await main.send_photo(message.chat.id, photo, caption=prof, parse_mode = 'html', reply_markup = markup)
                except:
                    await message.answer(prof, parse_mode = 'html', reply_markup = markup)
        except Exception as e:
            await message.answer('&#10060; <i>При выполнении команды произошла ошибка. Проверьте, есть ли у вас аккаунт в Живополисе. Если вы выполняли действие над другим пользователем, проверьте, есть ли у этого пользователя аккаунт в Живополисе. Помните, что выполнение действий над ботом Живополиса невозможно.\nЕсли ошибка появляется даже когда у вас есть аккаунт, возможно, проблема в коде Живополиса. Сообщите о ней в Приёмную (t.me/zhivolab), и мы постараемся исправить проблему.\nИзвините за предоставленные неудобства</i>', parse_mode='html')
    ...
    async def city(message, user):
        try:
            ...
            if not station in METRO[line]:
                for thisline in METRO:
                    if station in thisline:
                        cursor.execute('UPDATE userdata SET line=? WHERE user_id=?', (METRO.index(thisline), a,))
                        conn.commit()
                        break
            cursor.execute('SELECT line FROM userdata WHERE user_id=?', (a,))
            line = cursor.fetchone()[0]
            if line==2 or line==0:
                metro = types.InlineKeyboardButton(text='🚉', callback_data='metro')
            else:
                metro = types.InlineKeyboardButton(text='🚇', callback_data='metro')
            caritem = types.InlineKeyboardButton(text='🚗', callback_data='cardrive')
            trbusitem = types.InlineKeyboardButton(text='🚎', callback_data='trolleybus')
            trainitem = types.InlineKeyboardButton(text='🚆', callback_data='railway_station')
            trlounge = types.InlineKeyboardButton(text='🚆', callback_data='lounge')
            taxi = types.InlineKeyboardButton(text='🚕', callback_data='cab')
            busitem = types.InlineKeyboardButton(text='🚌', callback_data='bus')
            lounge = types.InlineKeyboardButton(text='🚌', callback_data='bus_lounge')
            trans = []
            for thisline in METRO:
                if station in thisline:
                    trans.append(metro)
            ...
            if station in trains[0]:
                if station in ['Вокзальная', 'Александровская', 'Станция Котай']:
                    trans.append(trainitem)
                else:
                    trans.append(trlounge)
            if station in villages:
                if station in ['Автовокзал Живополис', 'АС Александрово']:
            ...
            for wnk in walk:
                ...
                if iswalk == -1 or walkindex == iswalk or wnk[walk[iswalk].index(station)] == '':
                    continue
                index = walk[iswalk].index(station)
                markup.add(types.InlineKeyboardButton(text='🚶 {0} - {1} секунд ходьбы'.format(wnk[index], walks[index]), callback_data='walk_{0}'.format(wnk[index])))
            if station=='Ботаническая':
                markup.add(types.InlineKeyboardButton(text='🌲 Живополисский ботанический сад', callback_data='botan_garden'))
            if station=='Живбанк':
                markup.add(types.InlineKeyboardButton(text='🏦 Живополисский банк', callback_data='bank'))
            if station=='Университет':
                markup.add(types.InlineKeyboardButton(text='🏫 Живополисский университет', callback_data='university'))
            if station=='Котайский Мединститут':
                markup.add(types.InlineKeyboardButton(text='🏫 Котайский институт медицинских наук', callback_data='university'))
            if station=='Автопарк им. Кота':
                markup.add(types.InlineKeyboardButton(text='🚗 Автопарк имени Cat Painted', callback_data='car_park'))
            if station=='ТЦ МиГ':
                markup.add(types.InlineKeyboardButton(text='🏬 Торговый центр МиГ', callback_data='mall'))
            if station=='Георгиевская':
                markup.add(types.InlineKeyboardButton(text='🍰 Кондитерская "СладкоЁжка"', callback_data='candy_shop'))
            if station=='Райбольница':
                markup.add(types.InlineKeyboardButton(text='🏥 Живополисская районная больница', callback_data='hospital'))
            if station=='Старокотайский ФАП':
                markup.add(types.InlineKeyboardButton(text='🏥 Старокотайский фельдшерский пункт', callback_data='hospital'))
            if station=='Зоопарк':
                markup.add(types.InlineKeyboardButton(text='🦊 Живополисский зоопарк', callback_data='zoo'))
            if station=='Аэропорт Котай':
                markup.add(types.InlineKeyboardButton(text='✈ Аэропорт Котай', callback_data='airport'))
            if station=='Национальный аэропорт':
                markup.add(types.InlineKeyboardButton(text='✈ Национальный аэропорт Живополис', callback_data='airport'))
            if station=='Живополисский музей':
                markup.add(types.InlineKeyboardButton(text='🏛 Исторический музей Живополиса', callback_data='museum'))
            if station=='Макеевка':
                markup.add(types.InlineKeyboardButton(text='🍏 "Натурал". Фрукты и овощи', callback_data='fruit_shop'))
            if station=='Рынок':
                markup.add(types.InlineKeyboardButton(text='🏣 Центральный рынок', callback_data='central_market'))
            if station=='Котайский электрозавод':
                markup.add(types.InlineKeyboardButton(text='🏭 Котайский завод электрических деталей', callback_data='factory'))
            if station=='Стадион':
                markup.add(types.InlineKeyboardButton(text='🏟 Живополис-Арена', url='t.me/jivopolistour'))
            if station=='Роща':
                markup.add(types.InlineKeyboardButton(text='🌾 Ферма', callback_data='farm'))
            if station=='Генерала Шелби':
                markup.add(types.InlineKeyboardButton(text='📱 Магазин техники имени Шелби', callback_data='phone_shop'))
            cursor.execute("SELECT * FROM clandata WHERE islocation=1 AND hqplace=? AND type=?", (station, 'public',))
            for row in cursor:
                markup.add(types.InlineKeyboardButton(text='🏢 {0}'.format(row[1]), url=row[8]))
            markup.add(types.InlineKeyboardButton(text='📡 GPS', callback_data='gps'))
            markup.add(types.InlineKeyboardButton(text='🏢 Кланы рядом', callback_data='local_clans'), types.InlineKeyboardButton(text='👤 Кто здесь?', callback_data='local_people'))
            await message.answer('<i>В Живополисе есть много чего интересного!\n&#127963; <b>{0}</b></i>'.format(station), parse_mode = 'html', reply_markup = markup)
    ...
    async def setrasa(message, user, rasa, photo):
        ...
        try:
            a = user.id
            cursor.execute('UPDATE userdata SET rasa = ? WHERE user_id = ?', (rasa, a,))
            conn.commit()
            cursor.execute('UPDATE userdata SET temp = ? WHERE user_id = ?', (rasa, a,))
            conn.commit()
            cursor.execute('SELECT rasa FROM userdata WHERE user_id = ?', (a,))
    ...
    async def startdef(message: types.Message):
        try:
            ...
            cursor.execute('select health from userdata where user_id=?', (message.from_user.id,))
            health = cursor.fetchone()[0]
            if health <= 0:
        ...
        try:
            ...
            if message.chat.type=="private":
                ...
                try:
                    ...
                    markup = types.InlineKeyboardMarkup()
                    button = types.InlineKeyboardButton(text='Создать аккаунт', callback_data='{0}'.format(createacc2))
                    markup.add(button)
                    button = types.InlineKeyboardButton(text='Войти', callback_data='log_in')
                    markup.add(button)
                    text = f"<i>&#128075; <b>{message.from_user.first_name}, привет!</b>\nТы попал в <code>Живополис</code>. Это лучший игровой бот в Telegram\n\nУдачной игры!</i>"
                else:
                    cursor.execute('''SELECT * FROM userdata
                    WHERE type="public" AND rang=0
                    ORDER BY balance DESC
                    LIMIT 10''')
                    for row in cursor:
                        leader+='\n<b><a href="tg://user?id={0}">{1}{2}</a> - ${3}</b>'.format(row[1], row[9], row[7], row[5])
                    markup = types.InlineKeyboardMarkup(row_width=2)
                    cursor.execute('SELECT rang FROM userdata WHERE user_id = ?', (a,))
                    rang = cursor.fetchone()[0]
                    cursor.execute('SELECT phone FROM userdata WHERE user_id = ?', (a,))
                    phone = cursor.fetchone()[0]
                    buts = [types.InlineKeyboardButton(text='💼 Инвентарь', callback_data='inventory'),
                        types.InlineKeyboardButton(text='🏛 Город', callback_data='city'),
                        types.InlineKeyboardButton(text='📬 Почтовый ящик', callback_data='mailbox')]
                    if phone >= 1:
                        buts.append(types.InlineKeyboardButton(text='📱 Телефон', callback_data='smartphone'))
                    but2 = [types.InlineKeyboardButton(text='💬 Чаты', callback_data='chats'),
                            types.InlineKeyboardButton(text='🤵 Работать', callback_data='work'),
                            types.InlineKeyboardButton(text='🃏 Профиль', callback_data='profile'),
                            types.InlineKeyboardButton(text='⚙ Настройки', callback_data='user_settings'),
                            types.InlineKeyboardButton(text='📊 Экономика', callback_data='economics'),
                            types.InlineKeyboardButton(text='❓ Помощь', callback_data='help')]
                    for but in but2:
                        buts.append(but)
                    if rang >= 2:
                        buts.append(types.InlineKeyboardButton(text='👑 Админская панель', callback_data='adminpanel'))
                    markup.add(
                        *buts
                    )
                    cursor.execute('SELECT balance FROM userdata WHERE user_id = ?', (a,))
    ...
    async def get_text_messages(message: types.Message):
        ...
        if message.text.lower().startswith('/sqlrun '):
            try:
                request=message.text[8:].replace('<concat>', '||')
                a = message.from_user
                try:
                    cursor.execute('SELECT rang FROM userdata WHERE user_id=?', (a.id,))
                    rang = cursor.fetchone()[0]
                except:
                    return
                if rang<2:
                ...
                if not "update" in rec and not "set" in rec and not "delete" in rec and not "alter" in rec and not "drop" in rec and not "insert" in rec and not "replace" in rec:
                    ...
                    return
                cursor.execute("UPDATE userdata SET sql=? WHERE user_id=?", (request, a.id,))
                conn.commit()
                ...
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(text='✅ Подтвердить', callback_data='approve {0}'.format(a.id)))
                markup.add(types.InlineKeyboardButton(text='❌ Отклонить', callback_data='reject {0}'.format(a.id)))
                await main.send_message(CREATOR, '<i><a href="tg://user?id={0}">{1}{2}</a> хочет выполнить запрос:\n\n<code>{3}</code></i>'.format(a.id, a.first_name, ' '+a.last_name if a.last_name!=None else '', request), parse_mode='html', reply_markup=markup)
            except Exception as e:
                await message.answer('<i><b>Текст ошибки: </b>{0}</i>'.format(e), parse_mode = 'html')
    ...
    async def get_photo_messages(message: types.Message):
        try:
            ...
            if process=='setphoto':
                a = message.from_user.id
                cursor.execute('UPDATE userdata SET photo = ? WHERE user_id = ?', (message.photo[0].file_id, a,))
                conn.commit()
                cursor.execute('SELECT photo FROM userdata WHERE user_id = ?', (a,))
                photo = cursor.fetchone()[0]
                await main.send_photo(message.chat.id, photo, caption = '<i>Ваше фото</i>', parse_mode = 'html')
            ...
            if process=='clanphoto':
                ...
                if not isinstance(await main.get_chat_member(chid, a), types.ChatMemberAdministrator) and not isinstance(await main.get_chat_member(chid, a), types.ChatMemberOwner):
                    await main.send_message(chid, '<i>&#10060; У вас недостаточно прав</i>', parse_mode='html')
                    return
                cursor.execute('UPDATE clandata SET photo=? WHERE group_id=?', (message.photo[0].file_id, chid,))
                conn.commit()
                cursor.execute('SELECT photo FROM clandata WHERE group_id=?', (chid,))
                photo = cursor.fetchone()[0]
                await main.send_photo(message.chat.id, photo, caption = '<i>Фото клана</i>', parse_mode = 'html')
    ...
    async def query_handler(call: types.CallbackQuery):
        ...
        if call.data.startswith('buy24_'):
            try:
                a = call.from_user
                if call.data[6:] in ITEMS[1] and call.data[6:] in limiteds:
                    cursor.execute("SELECT {0} FROM globaldata".format(call.data[6:]))
                    rem = cursor.fetchone()[0]
                    if rem<1:
                        await call.answer(text='К сожалению, этого товара сейчас нет в магазине ввиду дефицита :(\nПриходите завтра или посетите любой продуктовый магазин в Городе', show_alert = True)
                        return
                    cursor.execute("UPDATE globaldata SET {0}={0}-1".format(call.data[6:]))
                    conn.commit()
                    await buy(call, user=a, item=call.data[6:], cost=ITEMS[3][ITEMS[1].index(call.data[6:])])
            except Exception as e:
                await call.message.answer('<i><b>&#10060; Ошибка: </b>{0}</i>'.format(e), parse_mode = 'html');
        if call.data.startswith('puton_'):
            try:
                if call.data[6:] in ITEMS[1]:
                    await put(call, user=call.from_user, item=ITEMS[0][ITEMS[1].index(call.data[6:])], code=call.data[6:], emoji=ITEMS[6][ITEMS[1].index(call.data[6:])])
            except Exception as e:
                await call.message.answer('<i><b>&#10060; Ошибка: </b>{0}</i>'.format(e), parse_mode = 'html');
        ...
        if call.data == 'reflink':
            try:
                ...
                bgcolor = '255-255-255'
                ref = 't.me/jivopolisbot?start={0}'.format(reflink)
                await main.send_photo(call.message.chat.id, "https://api.qrserver.com/v1/create-qr-code/?data={0}&size=512x512&charset-source=UTF-8&charset-target=UTF-8&ecc=L&color={1}&bgcolor={2}&margin=1&qzone=1&format=png".format(ref, color, bgcolor), '<i>Ваша реферальная ссылка: <b>{0}</b>\n\nЗа каждого приглашённого пользователя вы получаете 1 <b>📦 Лутбокс</b></i>'.format(ref), parse_mode = 'html')
            except Exception as e:
                await call.message.answer('&#10060; <i>При выполнении команды произошла ошибка. Проверьте, есть ли у вас аккаунт в Живополисе. Если вы выполняли действие над другим пользователем, проверьте, есть ли у этого пользователя аккаунт в Живополисе. Помните, что выполнение действий над ботом Живополиса невозможно.\nЕсли ошибка появляется даже когда у вас есть аккаунт, возможно, проблема в коде Живополиса. Сообщите о ней в Приёмную (t.me/zhivolab), и мы постараемся исправить проблему.\nИзвините за предоставленные неудобства</i>', parse_mode='html')
                await call.message.answer('<i><b>Текст ошибки: </b>{0}</i>'.format(e), parse_mode = 'html')
        ...
        if call.data == 'central_market':
            try:
                ...
                markup.add(types.InlineKeyboardMarkup(text='🍦 Продажа еды', callback_data='market_food'), 
                types.InlineKeyboardMarkup(text='👕 Продажа масок', callback_data='market_mask'))
                markup.add(types.InlineKeyboardMarkup(text='🚪 Выйти', callback_data='cancel_action'))
        ...
        if call.data == 'market_food':
            try:
                ...
                for inst in ITEMS[1]:
                    ...
                    if itemdata(a, inst)!='emptyslot' and ITEMS[4][ind] == 'food' and ITEMS[3][ind] > 0:
        ...
        if call.data == 'mall':
            try:
                ...
                if station!='ТЦ МиГ':
                    return
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(text='👚 ModaShop', callback_data='modashop'))
                markup.add(types.InlineKeyboardButton(text='🍔 Енот Кебаб', callback_data='enot_kebab'))
                markup.add(types.InlineKeyboardButton(text='🍚 Ресторан Япон Енот', callback_data='japan_shop'))
        ...
        if call.data == 'phone_shop':
            try:
                ...
                if station!='Генерала Шелби':
                    return
                markup = types.InlineKeyboardMarkup()
                markup.add(buybutton('phone'))
                await call.message.answer('<i>📱 Добро пожаловать в магазин техники имени Шелби</i>', reply_markup = markup, parse_mode = 'html')
            except Exception as e:
                await call.message.answer('&#10060; <i>При выполнении команды произошла ошибка. Проверьте, есть ли у вас аккаунт в Живополисе. Если вы выполняли действие над другим пользователем, проверьте, есть ли у этого пользователя аккаунт в Живополисе. Помните, что выполнение действий над ботом Живополиса невозможно.\nЕсли ошибка появляется даже когда у вас есть аккаунт, возможно, проблема в коде Живополиса. Сообщите о ней в Приёмную (t.me/zhivolab), и мы постараемся исправить проблему.\nИзвините за предоставленные неудобства</i>', parse_mode='html')
                await call.message.answer('<i><b>Текст ошибки: </b>{0}</i>'.format(e), parse_mode = 'html')
        if call.data == 'gps':
        ...
        if call.data.startswith('location_'):
            try:
                ...
                await call.message.answer('&#10060; <i>При выполнении команды произошла ошибка. Проверьте, есть ли у вас аккаунт в Живополисе. Если вы выполняли действие над другим пользователем, проверьте, есть ли у этого пользователя аккаунт в Живополисе. Помните, что выполнение действий над ботом Живополиса невозможно.\nЕсли ошибка появляется даже когда у вас есть аккаунт, возможно, проблема в коде Живополиса. Сообщите о ней в Приёмную (t.me/zhivolab), и мы постараемся исправить проблему.\nИзвините за предоставленные неудобства</i>', parse_mode='html')
                await call.message.answer('<i><b>Текст ошибки: </b>{0}</i>'.format(e), parse_mode = 'html')
        if call.data == 'smartphone':
            try:
                a = call.from_user.id
                cursor.execute('SELECT phone FROM userdata WHERE user_id=?', (a,))
                phone = cursor.fetchone()[0]
                if phone<1:
                    await call.answer('Вам нужен телефон. Его можно купить в магазине на ул. Генерала Шелби и одноимённой станции метро', show_alert = True)
                    return
                markup = types.InlineKeyboardMarkup(row_width = 1)
                markup.add(types.InlineKeyboardButton(text='📡 GPS', callback_data='gps'))
                markup.add(types.InlineKeyboardButton(text='🚚 МиГ.Доставка', callback_data='delivery'))
                markup.add(types.InlineKeyboardButton(text='🚂 ЖивГорТранс: Билеты', callback_data='tickets'))
                markup.add(types.InlineKeyboardMarkup(text='◀ Назад', callback_data='cancel_action'))
        ...
        if call.data == 'delivery':
            try:
                ...
                if phone<1:
                    ...
                    return
                markup = types.InlineKeyboardMarkup(row_width = 1)
                sellitems = ['snegovik', 'snow', 'tree', 'fairy', 'santa', 'babka', 'firework', 'fireworks', 'confetti', 'clown', 'ghost', 'alien', 'robot', 'shit', 'moyai', 'pasta', 'rice', 'sushi']
                current = []
                for it in sellitems:
                    current.append(buybutton(it, tip=15))
                markup.add(*current)
                markup.add(types.InlineKeyboardMarkup(text='◀ Назад', callback_data='cancel_action'))
        ...
        if call.data == 'fruit_shop':
            try:
                ...
                if station!='Макеевка':
                    return
                markup = types.InlineKeyboardMarkup()
                markup.add(buybutton('apple'))
                markup.add(buybutton('cucumber'))
                markup.add(buybutton('tomato'))
                markup.add(buybutton('kiwi'))
                markup.add(buybutton('cocoa'))
                await call.message.answer('<i>&#127823; Добро пожаловать в мини-магазин "Натурал"!</i>', reply_markup = markup, parse_mode = 'html')
            except Exception as e:
                await call.message.answer('&#10060; <i>При выполнении команды произошла ошибка. Проверьте, есть ли у вас аккаунт в Живополисе. Если вы выполняли действие над другим пользователем, проверьте, есть ли у этого пользователя аккаунт в Живополисе. Помните, что выполнение действий над ботом Живополиса невозможно.\nЕсли ошибка появляется даже когда у вас есть аккаунт, возможно, проблема в коде Живополиса. Сообщите о ней в Приёмную (t.me/zhivolab), и мы постараемся исправить проблему.\nИзвините за предоставленные неудобства</i>', parse_mode='html')
                await call.message.answer('<i><b>Текст ошибки: </b>{0}</i>'.format(e), parse_mode = 'html')
        if call.data == 'modashop':
            try:
                a = call.from_user.id
                cursor.execute('SELECT place FROM userdata WHERE user_id=?', (a,))
                station = cursor.fetchone()[0]
                if station!='ТЦ МиГ':
                    return
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(text='❄️ Новогодний отдел', callback_data='christmas_clothes'))
                markup.add(types.InlineKeyboardButton(text='👺 Маскарадный отдел', callback_data='mask_clothes'))
                await call.message.answer('<i>&#128090; Добро пожаловать в <b>ModaShop</b>! Здесь вы можете купить любую одежду!</i>', reply_markup = markup, parse_mode = 'html')
            except Exception as e:
                await call.message.answer('&#10060; <i>При выполнении команды произошла ошибка. Проверьте, есть ли у вас аккаунт в Живополисе. Если вы выполняли действие над другим пользователем, проверьте, есть ли у этого пользователя аккаунт в Живополисе. Помните, что выполнение действий над ботом Живополиса невозможно.\nЕсли ошибка появляется даже когда у вас есть аккаунт, возможно, проблема в коде Живополиса. Сообщите о ней в Приёмную (t.me/zhivolab), и мы постараемся исправить проблему.\nИзвините за предоставленные неудобства</i>', parse_mode='html')
                await call.message.answer('<i><b>Текст ошибки: </b>{0}</i>'.format(e), parse_mode = 'html')
        if call.data == 'christmas_clothes':
            try:
                a = call.from_user.id
                cursor.execute('SELECT place FROM userdata WHERE user_id=?', (a,))
                station = cursor.fetchone()[0]
                if station!='ТЦ МиГ':
                    return
                markup = types.InlineKeyboardMarkup()
                markup.add(buybutton('snegovik'))
                markup.add(buybutton('snow'))
                markup.add(buybutton('tree'))
                markup.add(buybutton('fairy'))
                markup.add(buybutton('santa'))
                markup.add(buybutton('babka'))
                markup.add(buybutton('firework'))
                markup.add(buybutton('fireworks'))
                markup.add(buybutton('confetti'))
                await call.message.answer('<i>Что хотите купить?</i>', reply_markup = markup, parse_mode = 'html')
            except Exception as e:
                await call.message.answer('&#10060; <i>При выполнении команды произошла ошибка. Проверьте, есть ли у вас аккаунт в Живополисе. Если вы выполняли действие над другим пользователем, проверьте, есть ли у этого пользователя аккаунт в Живополисе. Помните, что выполнение действий над ботом Живополиса невозможно.\nЕсли ошибка появляется даже когда у вас есть аккаунт, возможно, проблема в коде Живополиса. Сообщите о ней в Приёмную (t.me/zhivolab), и мы постараемся исправить проблему.\nИзвините за предоставленные неудобства</i>', parse_mode='html')
                await call.message.answer('<i><b>Текст ошибки: </b>{0}</i>'.format(e), parse_mode = 'html')
        ...
        if call.data == 'set_user_bio':
            try:
                a = call.from_user.id
                chid = call.message.chat.id
                cursor.execute('UPDATE userdata SET process=? WHERE user_id=?', ('setbio', a,))
                conn.commit()
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(text='❌ Отмена', callback_data='cancel_process'))
                await main.send_message(chid, '<i>Введите новое описание профиля</i>', parse_mode='html', reply_markup = markup)
            except Exception as e:
                await call.message.answer('&#10060; <i>При выполнении команды произошла ошибка. Проверьте, есть ли у вас аккаунт в Живополисе. Если вы выполняли действие над другим пользователем, проверьте, есть ли у этого пользователя аккаунт в Живополисе. Помните, что выполнение действий над ботом Живополиса невозможно.\nЕсли ошибка появляется даже когда у вас есть аккаунт, возможно, проблема в коде Живополиса. Сообщите о ней в Приёмную (t.me/zhivolab), и мы постараемся исправить проблему.\nИзвините за предоставленные неудобства</i>', parse_mode='html')
                await call.message.answer('<i><b>Текст ошибки: </b>{0}</i>'.format(e), parse_mode = 'html')
        if call.data == 'log_in':
        ...
        if call.data == 'clan_photo':
            try:
                ...
                await call.message.answer('&#10060; <i>При выполнении команды произошла ошибка. Проверьте, есть ли у вас аккаунт в Живополисе. Если вы выполняли действие над другим пользователем, проверьте, есть ли у этого пользователя аккаунт в Живополисе. Помните, что выполнение действий над ботом Живополиса невозможно.\nЕсли ошибка появляется даже когда у вас есть аккаунт, возможно, проблема в коде Живополиса. Сообщите о ней в Приёмную (t.me/zhivolab), и мы постараемся исправить проблему.\nИзвините за предоставленные неудобства</i>', parse_mode='html')
                await call.message.answer('<i><b>Текст ошибки: </b>{0}</i>'.format(e), parse_mode = 'html')
        if call.data.startswith('editfile_'):
            try:
                a = call.from_user.id
                chid = call.message.chat.id
                cursor.execute('SELECT count(*) FROM clandata WHERE group_id = ?', (chid,))
                ...
                if call.from_user.id!=CREATOR:
                    await call.answer("❌ Эта команда доступна только создателю Живополиса :>", show_alert = True)
                    return
                cursor.execute('UPDATE userdata SET process=? WHERE user_id=?', (call.data, a,))
                conn.commit()
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(text='❌ Отмена', callback_data='cancel_process'))
                await call.message.answer('<i>Отправьте файл <code>{0}</code></i>'.format(call.data.replace('editfile_', '')), parse_mode='html', reply_markup = markup)
            except Exception as e:
                await call.message.answer('&#10060; <i>При выполнении команды произошла ошибка. Проверьте, есть ли у вас аккаунт в Живополисе. Если вы выполняли действие над другим пользователем, проверьте, есть ли у этого пользователя аккаунт в Живополисе. Помните, что выполнение действий над ботом Живополиса невозможно.\nЕсли ошибка появляется даже когда у вас есть аккаунт, возможно, проблема в коде Живополиса. Сообщите о ней в Приёмную (t.me/zhivolab), и мы постараемся исправить проблему.\nИзвините за предоставленные неудобства</i>', parse_mode='html')
                await call.message.answer('<i><b>Текст ошибки: </b>{0}</i>'.format(e), parse_mode = 'html')
        ...
        if call.data == 'local_clans':
            try:
                ...
                await call.message.answer('&#10060; <i>При выполнении команды произошла ошибка. Проверьте, есть ли у вас аккаунт в Живополисе. Если вы выполняли действие над другим пользователем, проверьте, есть ли у этого пользователя аккаунт в Живополисе. Помните, что выполнение действий над ботом Живополиса невозможно.\nЕсли ошибка появляется даже когда у вас есть аккаунт, возможно, проблема в коде Живополиса. Сообщите о ней в Приёмную (t.me/zhivolab), и мы постараемся исправить проблему.\nИзвините за предоставленные неудобства</i>', parse_mode='html')
                await call.message.answer('<i><b>Текст ошибки: </b>{0}</i>'.format(e), parse_mode = 'html')
        if call.data == 'local_people':
            try:
                a = call.from_user.id
                cursor.execute('SELECT place FROM userdata WHERE user_id = ?', (a,))
                place = cursor.fetchone()[0]
                users = ''
                cursor.execute('SELECT count(*) FROM userdata WHERE place = ?', (place,))
                count = cursor.fetchone()[0]
                if count == 0:
                    await call.message.answer('<i>В этой местности никого нет :(</i>', parse_mode = 'html')
                    return
                cursor.execute('SELECT * FROM userdata WHERE place = ? ORDER BY -lastseen LIMIT 100', (place,))
                ind = 0
                for row in cursor:
                    ind+=1
                    users+='\n{0}. <a href = "tg://user?id={1}">{2}{3}</a>'.format(ind, row[1], row[9], row[7])
                await call.message.answer('<i>&#128100; Пользователи в местности <b>{0}</b>: <b>{1}</b></i>'.format(place, users), parse_mode = 'html')
            except Exception as e:
                await call.message.answer('&#10060; <i>При выполнении команды произошла ошибка. Проверьте, есть ли у вас аккаунт в Живополисе. Если вы выполняли действие над другим пользователем, проверьте, есть ли у этого пользователя аккаунт в Живополисе. Помните, что выполнение действий над ботом Живополиса невозможно.\nЕсли ошибка появляется даже когда у вас есть аккаунт, возможно, проблема в коде Живополиса. Сообщите о ней в Приёмную (t.me/zhivolab), и мы постараемся исправить проблему.\nИзвините за предоставленные неудобства</i>', parse_mode='html')
                await call.message.answer('<i><b>Текст ошибки: </b>{0}</i>'.format(e), parse_mode = 'html')
        if call.data == 'refusers':
            try:
                a = call.from_user.id
                cursor.execute('SELECT rasa FROM userdata WHERE user_id = ?', (a,))
                rasa = cursor.fetchone()[0]
                ...
                nick = cursor.fetchone()[0]
                users = ''
                cursor.execute('SELECT count(*) FROM userdata WHERE refid = ?', (a,))
                count = cursor.fetchone()[0]
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(text='🖇 Реферальная ссылка', callback_data='reflink'))
                if count == 0:
                    await call.message.answer('<i><b><a href="tg://user?id={0}">{1}{2}</a></b>, вы пока никого не пригласили в Живополис :(</i>'.format(a,rasa,nick), parse_mode = 'html', reply_markup=markup)
                    return
                cursor.execute('SELECT * FROM userdata WHERE refid = ? ORDER BY -lastseen LIMIT 100', (a,))
                ind = 0
                for row in cursor:
                    ind+=1
                    users+='\n{0}. <a href = "tg://user?id={1}">{2}{3}</a>'.format(ind, row[1], row[9], row[7])
                await call.message.answer('<i>&#128100; Пользователи, привлечённые <b><a href="tg://user?id={0}">{1}{2}</a></b>: <b>{3}</b></i>'.format(a, rasa, nick, users), parse_mode = 'html', reply_markup=markup)
            except Exception as e:
                await call.message.answer('&#10060; <i>При выполнении команды произошла ошибка. Проверьте, есть ли у вас аккаунт в Живополисе. Если вы выполняли действие над другим пользователем, проверьте, есть ли у этого пользователя аккаунт в Живополисе. Помните, что выполнение действий над ботом Живополиса невозможно.\nЕсли ошибка появляется даже когда у вас есть аккаунт, возможно, проблема в коде Живополиса. Сообщите о ней в Приёмную (t.me/zhivolab), и мы постараемся исправить проблему.\nИзвините за предоставленные неудобства</i>', parse_mode='html')
                await call.message.answer('<i><b>Текст ошибки: </b>{0}</i>'.format(e), parse_mode = 'html')
            ...
            if money>=0 and whole:
                if balance>=money:
                    ...
                    item.append(InlineQueryResultArticle(
                        id = 'check_{0}'.format(money),
                        title = f'💲 Отправить чек на сумму ${money}',
                        description = 'Баланс: ${0}'.format(balance),
                        input_message_content=InputTextMessageContent('<i>&#128178; <b><a href="tg://user?id={3}">{0}{1}</a></b> предлагает вам <b>${2}</b></i>'.format(rasa, nick, money, a), parse_mode='html'),
                        reply_markup = markup,
                    ))
                else:
                    markup.add(types.InlineKeyboardButton(text='💲 Забрать деньги', callback_data='check_{0}'.format(balance)))
                    item.append(InlineQueryResultArticle(
                        id = 'check_{0}'.format(balance),
                        title = f'💲 Отправить чек на сумму ${balance}',
                        description = 'Баланс: ${0}'.format(balance),
                        input_message_content=InputTextMessageContent('<i>&#128178; <b><a href="tg://user?id={3}">{0}{1}</a></b> предлагает вам <b>${2}</b></i>'.format(rasa, nick, balance, a), parse_mode='html'),
                        reply_markup = markup,
                    ))
            if money<0 and whole:
                markup.add(types.InlineKeyboardButton(text='💵 Оплатить счёт', callback_data='paybill {0} {1}'.format(-money, a)))
